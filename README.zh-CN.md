...
**记忆管理层：**
- Reflection 会把受管事实写进各核心记忆文件的显式 managed region
- managed region 外的自由文本仍然归用户控制，proposal apply 时不会被改写
- 每条 managed fact 都会在本地保留 provenance 和 lifecycle history
- 变更通过 proposal 创建、审阅、显式 apply/discard 完成
...
**受管事实存储**
Reflection 还会在 `<workspaceDir>/.openclaw-reflection/` 下持久化 managed facts、生命周期事件和 proposals
...
**命令查询**
`reflection` 命令提供以文件为中心的记忆管理流程：
- `reflection files` 列出 managed files 和健康状态
- `reflection file <file>` 查看 facts 和 drift 状态
- `reflection fact <fact-id>` 查看 provenance 和 lifecycle
- `reflection proposal|apply|discard ...` 审阅并执行 proposal
- `reflection reconcile <file> --mode ...` 处理 managed-region drift
...
- 以文件为中心的记忆检查，以及 proposal 驱动的 delete/edit/move 流程
- managed-region drift 检测与 reconcile 命令

**暂不支持：**
- 多 agent 之间的记忆协调
- 多 agent 环境下的按 agent 分流管理
- 自动把 managed region 外的任意自由文本收编为 facts
- agent 自主 apply proposal
